...
/**
 * R2-1 on-chain (blockchain.info BTC 24h stats):
 *   GET /api/desk/onchain → market_price_usd, hash_rate, n_tx, etc.
 * Python CLI is the single source of truth (cli markets-onchain --json).
 */
export async function GET() {
  ...
  const cliArgs = [
    "-m", "gold_desk.cli", "markets-onchain", "--json",
  ...
  const result = await new Promise<Record<string, unknown>>((resolve) => {
    execFile(
      PYTHON,
      cliArgs,
      { cwd: HARNESS, env: { ...process.env, PYTHONPATH: path.join(HARNESS, "src") }, timeout: 15_000, maxBuffer: 2 * 1024 * 1024 },
      (err, stdout) => {
        if (err && !stdout) { resolve({ ok: false, error: err.message }); return; }
        try { resolve(JSON.parse(stdout.trim().split("\n").pop() || "{}")); }
        catch { resolve({ ok: false, error: "unparseable onchain output" }); }
      },
    );
  });
