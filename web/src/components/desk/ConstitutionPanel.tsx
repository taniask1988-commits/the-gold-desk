...
export function LawsMarquee() {
  ...
  return (
    <div className="overflow-hidden border-t border-white/[0.07] bg-white/[0.02] py-2 backdrop-blur-xl">
      <div className="gdc-marquee-track">
        {items.map(([id, text], i) => (
          <span key={i} className="mx-6 flex items-center gap-2 text-[9px] tracking-[0.12em]">
            <span className="font-bold text-[#e8b440]">{id}</span>
      ...
      <div className="grid gap-3 p-3 md:grid-cols-2">
        <div className="rounded-2xl border border-white/[0.05] bg-white/[0.02] p-4 backdrop-blur-sm">
          ...
          <div className="space-y-1.5 text-[10.5px]">
            ...
            <div className="mt-2 rounded-lg border border-[#e8b440]/25 bg-[#e8b440]/[0.06] p-2.5 text-[9.5px] leading-relaxed text-[#d29922] backdrop-blur-sm">
          ...
          <div className="gdc-scroll max-h-[168px] space-y-[3px] overflow-y-auto pr-1">
            {LAWS.map(([id, text]) => (
              <div key={id} className="flex gap-2 text-[9.5px] leading-relaxed">
                <span className="w-5 shrink-0 font-bold text-[#e8b440]">{id}</span>
                <span className="text-[#a5afba]">{text}</span>
              </div>
            ))}
          </div>
