...
export function TicketBoard({ tickets }: { tickets: TicketDTO[] }) {
  return (
    <div className="gdc-panel overflow-hidden">
      ...
      <div className="gdc-scroll max-h-[420px] space-y-2 overflow-y-auto p-3">
        ...
        {tickets.map((t) => {
          ...
          const st = stageOf(t);
          return (
            <div key={t.ticket_id} className="rounded-xl border border-white/[0.08] bg-white/[0.03] p-3 backdrop-blur-sm transition-all hover:-translate-y-px hover:border-white/[0.14] hover:bg-white/[0.045]">
                ...
                {STAGES.map((s, i) => (
                  <div key={s} className="flex items-center gap-1.5">
                    <div
                      ...
                      style={{ background: i <= st ? color : "#1b222b", boxShadow: i === st ? `0 0 8px ${color}66` : "none" }}
                    />
                    {i < STAGES.length - 1 && (
